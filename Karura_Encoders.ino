 ...
 * Company: CUI Inc.
 * Author: Jason Kelly; Edits by Johan Kornet
 * Version: 1.0.0.0
 * Date: August 8, 2016; February 21, 2026
 *
...
void setup()
{
  ...
  delay(500);  // Wait for AMT20 to complete initialization
}
...
void loop()
{
  ...
  while(true)
  {
    ...
    float DegPosition = 360*(float(currentPosition)/4096);
    Serial.println(DegPosition);
...
uint8_t SPIWrite(uint8_t sendByte)
{
  ...
  delayMicroseconds(20);  // Changed from 10 to 20 per datasheet recommendation
